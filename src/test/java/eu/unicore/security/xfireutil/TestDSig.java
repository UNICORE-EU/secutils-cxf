...
public class TestDSig extends AbstractTestBase
{
	...
	public void testNormalDSig2()
	{
		try
		{
			...
			MockSecurityConfig config = new MockSecurityConfig(
					false, true, true);
			config.getExtraSettings().setProperty(XFireClientFactory.LOG_INCOMING, "true");
			config.getExtraSettings().setProperty(XFireClientFactory.LOG_OUTGOING, "true");
	...
	{
		Set<String> toSign = UnicoreXFireClientFactory.getOperationsToSign(
